...
/**
 * Manifest files and vendored directories for each ecosystem. Used to
 * generate path-relative exclusions: after finding manifests, vendored
 * dirs are excluded relative to each manifest's directory. See #3814.
 *
 * NOTE: These manifest lists are intentionally separate from the file
 * patterns used in each create*Bom() function. VENDORED_DIRS needs broad
 * recursive detection to find every project boundary, while BOM creation
 * uses conditional fallback chains, root-only checks, and derived paths.
 * When adding a new ecosystem or manifest type, update both this config
 * and the corresponding create*Bom() / createXBom() detection logic.
 */
const VENDORED_DIRS = {
  js: { manifests: ["package.json"], dirs: ["node_modules"] },
  java: {
    manifests: ["pom.xml", "build.gradle*", "build.mill", "build.sbt"],
    dirs: ["target", ".gradle"],
  },
  py: {
    manifests: [
    ...
    dirs: ["venv", ".venv", ".tox", "__pypackages__"],
  },
  go: { manifests: ["go.mod"], dirs: ["vendor"] },
  rust: { manifests: ["Cargo.toml"], dirs: ["target"] },
  php: { manifests: ["composer.json"], dirs: ["vendor"] },
  ruby: {
    manifests: ["Gemfile", "*.gemspec"],
    dirs: ["vendor/bundle", ".bundle"],
  },
  csharp: { manifests: ["*.csproj", "*.sln"], dirs: ["bin", "obj"] },
  dart: { manifests: ["pubspec.yaml"], dirs: [".dart_tool"] },
  swift: { manifests: ["Package.swift"], dirs: [".build"] },
  cocoa: { manifests: ["Podfile"], dirs: ["Pods"] },
  cpp: {
    manifests: ["CMakeLists.txt", "conanfile.txt", "conan.lock", "meson.build"],
    dirs: ["build"],
  },
  clojure: { manifests: ["deps.edn", "project.clj"], dirs: ["target"] },
  elixir: { manifests: ["mix.exs", "mix.lock"], dirs: ["deps", "_build"] },
};

/**
 * Exclude an ecosystem's vendored directories from other ecosystem scanners.
 * Scans for manifest files to determine where vendored dirs are, then
 * generates path-relative exclusion patterns. Skipped for OCI scans.
 *
 * @param {string} ecosystem Key into VENDORED_DIRS
 * @param {string} scanPath Root directory being scanned
 * @param {Object} options CLI options (mutated in place)
 */
export function excludeVendoredDirs(ecosystem, scanPath, options) {
  if (hasAnyProjectType(["oci"], options, false)) {
    return;
  }
  const config = VENDORED_DIRS[ecosystem];
  if (!config?.manifests?.length || !config?.dirs?.length) {
    return;
  }
  if (!options.ecosystemExcludes) {
    options.ecosystemExcludes = {};
  }
  if (!options.ecosystemExcludes[ecosystem]) {
    options.ecosystemExcludes[ecosystem] = [];
  }
  const patterns = options.ecosystemExcludes[ecosystem];
  for (const manifestPattern of config.manifests) {
    const manifestFiles = getAllFiles(
      scanPath,
      `**/${manifestPattern}`,
      options,
    );
    for (const manifestFile of manifestFiles) {
      // Filter out manifests found inside directories already covered by
      // exclusion patterns (from this or other ecosystems). These are
      // vendored dependencies, not project boundaries. See #3814.
      // NOTE: filtering is done inline (not pre-computed) so that patterns
      // added by earlier manifests in this loop are visible when checking
      // later ones (e.g. root package.json adds node_modules/**, then
      // node_modules/moment-timezone/package.json is skipped).
      const mfDir = relative(scanPath, manifestFile).replaceAll(sep, "/");
      let skip = false;
      // Check against patterns already generated in this call
      for (const p of patterns) {
        const dirPrefix = p.replace(/\/\*\*$/, "/");
        if (mfDir.startsWith(dirPrefix)) {
          skip = true;
          break;
        }
      }
      // Check against other ecosystems' exclusions
      if (!skip && options.ecosystemExcludes) {
        outer: for (const [eco, ecoPatterns] of Object.entries(
          options.ecosystemExcludes,
        )) {
          if (eco === ecosystem) continue;
          for (const p of ecoPatterns) {
            const dirPrefix = p.replace(/\/\*\*$/, "/");
            if (mfDir.startsWith(dirPrefix)) {
              skip = true;
              break outer;
      ...
      const manifestDir = relative(scanPath, dirname(manifestFile));
      for (const dir of config.dirs) {
        const pattern = (
          manifestDir ? join(manifestDir, dir, "**") : join(dir, "**")
        ).replaceAll(sep, "/");
...
export async function createMultiXBom(pathList, options) {
  ...
  for (const path of pathList) {
    ...
    if (hasAnyProjectType(["oci", "js"], options)) {
      ...
      if (bomData?.bomJson?.components?.length) {
        excludeVendoredDirs("js", path, options);
      }
    }
    // Java
    if (hasAnyProjectType(["oci", "java"], options)) {
      ...
      if (bomData?.bomJson?.components?.length) {
        excludeVendoredDirs("java", path, options);
      }
    }
    if (hasAnyProjectType(["oci", "py"], options)) {
      ...
      if (bomData?.bomJson?.components?.length) {
        excludeVendoredDirs("py", path, options);
      }
    }
    if (hasAnyProjectType(["oci", "go"], options)) {
      ...
      if (bomData?.bomJson?.components?.length) {
        excludeVendoredDirs("go", path, options);
      }
    }
    if (hasAnyProjectType(["oci", "rust"], options)) {
      ...
      if (bomData?.bomJson?.components?.length) {
        excludeVendoredDirs("rust", path, options);
      }
    }
    if (hasAnyProjectType(["oci", "php"], options)) {
      ...
      if (bomData?.bomJson?.components?.length) {
        excludeVendoredDirs("php", path, options);
      }
    }
    if (hasAnyProjectType(["oci", "ruby"], options)) {
      ...
      if (bomData?.bomJson?.components?.length) {
        excludeVendoredDirs("ruby", path, options);
      }
    }
    if (hasAnyProjectType(["oci", "csharp"], options)) {
      ...
      options.currentEcosystem = "csharp";
      ...
      if (bomData?.bomJson?.components?.length) {
        excludeVendoredDirs("csharp", path, options);
      }
    }
    if (hasAnyProjectType(["oci", "dart"], options)) {
      ...
      if (bomData?.bomJson?.components?.length) {
        excludeVendoredDirs("dart", path, options);
      }
    }
    if (hasAnyProjectType(["oci", "haskell"], options)) {
      ...
      options.currentEcosystem = "haskell";
    ...
    if (hasAnyProjectType(["oci", "elixir"], options)) {
      ...
      options.currentEcosystem = "elixir";
      ...
      if (bomData?.bomJson?.components?.length) {
        excludeVendoredDirs("elixir", path, options);
      }
    }
    if (hasAnyProjectType(["oci", "c"], options)) {
      ...
      if (bomData?.bomJson?.components?.length) {
        excludeVendoredDirs("cpp", path, options);
      }
    }
    if (hasAnyProjectType(["oci", "clojure"], options)) {
      ...
      options.currentEcosystem = "clojure";
      ...
      if (bomData?.bomJson?.components?.length) {
        excludeVendoredDirs("clojure", path, options);
      }
    }
    if (hasAnyProjectType(["oci", "github"], options)) {
      ...
      options.currentEcosystem = "github";
    ...
    if (hasAnyProjectType(["oci", "cloudbuild"], options)) {
      ...
      options.currentEcosystem = "cloudbuild";
    ...
    if (hasAnyProjectType(["oci", "swift"], options)) {
      ...
      options.currentEcosystem = "swift";
      ...
      if (bomData?.bomJson?.components?.length) {
        excludeVendoredDirs("swift", path, options);
      }
    }
    ...
    if (hasAnyProjectType(["oci", "cocoa"], options)) {
      options.currentEcosystem = "cocoa";
      ...
      if (bomData?.bomJson?.components?.length) {
        excludeVendoredDirs("cocoa", path, options);
      }
    }
    if (hasAnyProjectType(["oci", "nix"], options)) {
      options.currentEcosystem = "nix";
