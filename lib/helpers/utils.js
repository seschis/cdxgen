...
export function getAllFiles(dirPath, pattern, options = {}) {
  ...
  // Apply ecosystem-tagged exclusions from other ecosystems.
  // Each ecosystem's vendored dirs are excluded for all scanners except
  // the owning ecosystem. See #3814.
  if (
    options?.ecosystemExcludes &&
    typeof options.ecosystemExcludes === "object"
  ) {
    for (const [eco, patterns] of Object.entries(options.ecosystemExcludes)) {
      if (eco !== options.currentEcosystem && Array.isArray(patterns)) {
        ignoreList = ignoreList.concat(patterns);
      }
    }
  }
  const includeDot =
