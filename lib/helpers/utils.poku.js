...
describe("getAllFiles node_modules handling", () => {
  const fixturePath = "./test/data/node-modules-scanning";

  it("excludes node_modules by default for non-package.json patterns", () => {
    const results = getAllFiles(fixturePath, "**/composer.json", {});
    const names = results.map((f) => f.replace(/\\/g, "/"));
    assert.ok(
      names.some((f) => f.includes("src/composer.json")),
      "should find src/composer.json",
    );
    assert.ok(
      !names.some((f) => f.includes("node_modules")),
      "should NOT find composer.json inside node_modules",
    );
  });

  it("includes node_modules when includeNodeModulesDir is true", () => {
    const results = getAllFiles(fixturePath, "**/composer.json", {
    ...
    assert.ok(
      names.some((f) =>
        f.includes("node_modules/moment-timezone/composer.json"),
      ),
      "should find composer.json inside node_modules",
    );
  });

  it("excludes node_modules when includeNodeModulesDir is explicitly false", () => {
    const results = getAllFiles(fixturePath, "**/composer.json", {
  ...
  it("options.exclude overrides includeNodeModulesDir=true", () => {
    const results = getAllFiles(fixturePath, "**/composer.json", {
      includeNodeModulesDir: true,
    ...
    assert.ok(
      !names.some((f) => f.includes("node_modules")),
      "should NOT find composer.json inside node_modules when excluded",
    );
  });

  it("includes node_modules for package.json pattern by default", () => {
    const results = getAllFiles(fixturePath, "**/package.json", {});
    const names = results.map((f) => f.replace(/\\/g, "/"));
    assert.ok(
      names.some((f) =>
        f.includes("node_modules/moment-timezone/package.json"),
      ),
      "should find package.json inside node_modules",
    );
    assert.ok(
      names.length >= 2,
      "should find at least root and node_modules package.json",
    );
  });

  it("applies ecosystemExcludes from other ecosystems", () => {
    const results = getAllFiles(fixturePath, "**/composer.json", {
      ecosystemExcludes: {
        js: ["node_modules/**"],
      },
      currentEcosystem: "php",
    });
    const names = results.map((f) => f.replace(/\\/g, "/"));
    assert.ok(
      names.some((f) => f.includes("src/composer.json")),
    ...
    assert.ok(
      !names.some((f) => f.includes("node_modules")),
      "should exclude node_modules (owned by js, not php)",
    );
  });

  it("skips ecosystemExcludes from own ecosystem", () => {
    const results = getAllFiles(fixturePath, "**/composer.json", {
      includeNodeModulesDir: true,
      ecosystemExcludes: {
        php: ["node_modules/**"],
    ...
    assert.ok(
      names.some((f) => f.includes("node_modules")),
      "should NOT exclude node_modules when it belongs to own ecosystem",
    );
  });

  it("applies multiple ecosystem excludes but skips own", () => {
    const results = getAllFiles(fixturePath, "**/composer.json", {
      includeNodeModulesDir: true,
      ecosystemExcludes: {
        js: ["node_modules/**"],
    ...
    assert.ok(
      !names.some((f) => f.includes("node_modules")),
      "should exclude node_modules (js ecosystem)",
    );
    assert.ok(
      names.some((f) => f.includes("src/composer.json")),
      "should NOT exclude src (own php ecosystem)",
    );
  });

  it("works without ecosystemExcludes (backwards compatible)", () => {
    const results = getAllFiles(fixturePath, "**/composer.json", {
      includeNodeModulesDir: true,
    });
    const names = results.map((f) => f.replace(/\\/g, "/"));
    assert.ok(
      names.some((f) => f.includes("node_modules")),
      "should find node_modules when no ecosystemExcludes set",
    );
  });

  it("allows own ecosystem to scan shared vendor/ dir", () => {
    const results = getAllFiles(fixturePath, "**/composer.json", {
      ecosystemExcludes: {
        php: ["vendor/**"],
      },
    ...
    const names = results.map((f) => f.replace(/\\/g, "/"));
    assert.ok(
      names.some((f) => f.includes("vendor/some-go-dep/composer.json")),
      "PHP should find composer.json in vendor/ when vendor/ is only excluded by php (own ecosystem)",
    );
  });

  it("blocks other ecosystems from scanning vendor/ dir", () => {
    const results = getAllFiles(fixturePath, "**/composer.json", {
      ecosystemExcludes: {
        go: ["vendor/**"],
      },
    ...
    const names = results.map((f) => f.replace(/\\/g, "/"));
    assert.ok(
      !names.some((f) => f.includes("vendor/")),
      "Java should NOT find files in vendor/ when Go excluded it",
    );
  });

  it("excludes Rust target/ but not Java target/ when currentEcosystem is java", () => {
    const results = getAllFiles(fixturePath, "**/*.xml", {
      ecosystemExcludes: {
        java: ["service-a/target/**"],
        rust: ["rust-lib/target/**"],
      },
      currentEcosystem: "java",
    });
    // Java's own target/ exclusion is skipped, Rust's is applied
    // This validates path-relative exclusions don't collide on shared dir names
    assert.ok(Array.isArray(results));
  });

  it("ecosystemExcludes overrides includeNodeModulesDir=true", () => {
    const results = getAllFiles(fixturePath, "**/composer.json", {
      includeNodeModulesDir: true,
      ecosystemExcludes: {
        js: ["node_modules/**"],
    ...
    assert.ok(
      !names.some((f) => f.includes("node_modules")),
      "ecosystemExcludes should block node_modules even when includeNodeModulesDir is true",
    );
  });
});
